...
import com.trip.nesgame.doubledragon.input.*;
import com.trip.nesgame.doubledragon.wrapper.Wrapper;

public class EmulatorActivity extends Activity implements
		Emulator.FrameUpdateListener,
		SharedPreferences.OnSharedPreferenceChangeListener,
		SurfaceHolder.Callback, View.OnTouchListener,
		EmulatorView.OnTrackballListener, Emulator.OnFrameDrawnListener,
		GameKeyListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		final String[] prefKeys = { "fullScreenMode", "flipScreen",
				"fastForwardSpeed", "frameSkipMode", "maxFrameSkips",
				"refreshRate", "soundEnabled", "soundVolume",
				"accurateRendering", "secondController", "enableTrackball",
				"trackballSensitivity", "enableSensor", "sensorSensitivity",
				"enableVKeypad", "scalingMode", "aspectRatio", "enableCheats",
				"orientation", "useInputMethod", "quickLoad", "quickSave",
	...
	public boolean onPrepareOptionsMenu(Menu menu) {
		...
		menu.findItem(R.id.menu_cheats)
				.setEnabled(emulator.getCheats() != null);
	...
	public void onSharedPreferenceChanged(SharedPreferences prefs, String key) {
		if (key.startsWith("gamepad")) {
			...
			fastForwardSpeed = Float.parseFloat(value.substring(0,
					value.length() - 1));
			if (inFastForward)
			...
			emulatorView
					.setOnTrackballListener(prefs.getBoolean(key, true) ? this
							: null);
	...
	public void onFrameDrawn(Canvas canvas) {
		/*Bitmap bitmap = ((BitmapDrawable) getResources().getDrawable(R.drawable.button_a)).getBitmap();
		bitmap = Bitmap.createScaledBitmap(bitmap, 47, 47, true);
...
*/
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		if (event.getAction() == MotionEvent.ACTION_DOWN) {
			...
			int y = (int) event.getY() * surfaceHeight
					/ emulatorView.getHeight();
			if (flipScreen) {
				x = surfaceWidth - x;
				y = surfaceHeight - y;
			}
	...
	private Dialog createWifiConnectDialog() {
		DialogInterface.OnClickListener l = new DialogInterface.OnClickListener() {
			public void onClick(DialogInterface dialog, int which) {
				...
				String ip = ((TextView) d.findViewById(R.id.ip_address))
						.getText().toString();
				String port = ((TextView) d.findViewById(R.id.port)).getText()
						.toString();
	...
	private boolean isROMSupported(String file) {
		...
		String[] filters = getResources().getStringArray(
				R.array.file_chooser_filters);
	...
	private boolean loadROM() {
		...
		if (!isROMSupported(path)) {
			Toast.makeText(this, R.string.rom_not_supported, Toast.LENGTH_SHORT)
					.show();
			finish();
			return false;
		}
	...
	private void onWifiServer() {
		...
		try {
			addr = InetAddress
					.getByAddress(new byte[] { (byte) ip, (byte) (ip >>> 8),
	...
	private void onWifiConnect(String ip, String portStr) {
		...
		if (port <= 0) {
			Toast.makeText(this, R.string.invalid_port, Toast.LENGTH_SHORT)
					.show();
			return;
		}
	...
	private void onChangeDisk() {
		...
		for (int i = 0; i < items.length; i++) {
			items[i] = getString(R.string.disk_side, i / 2 + 1,
					(i % 2) == 0 ? 'A' : 'B');
		}
